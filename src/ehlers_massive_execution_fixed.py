#!/usr/bin/env python3
"""
Ehlers Massive Execution - Fixed Version with Proper Data Loading
...
class EhlersMassiveExecutionFixed:
    ...
    def get_all_symbols(self) -> List[str]:
        ...
        if data_path.exists():
            ...
            print(f"Found {len(symbol_dirs)} symbol directories")
        ...
        print(f"Total unique symbols found: {len(symbols)}")
    ...
    def load_symbol_data(self, symbol: str) -> Optional[Dict[str, np.ndarray]]:
        ...
        try:
            ...
            if not all(col in df.columns for col in required_columns):
                print(f"Missing required columns for {symbol}: {df.columns.tolist()}")
                return None
            ...
            if len(data['close']) < 50:  # Need at least 50 data points
                return None
...
def main():
    ...
    print("EHLERS MASSIVE EXECUTION - FIXED VERSION")
    print("=" * 60)
    
    # Initialize execution engine
    executor = EhlersMassiveExecutionFixed()
