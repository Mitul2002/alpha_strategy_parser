#!/usr/bin/env python3
"""
Limited Ehlers Execution - Run all 50 strategies on a subset of symbols including RELIANCE
"""
...
class EhlersLimitedExecution:
    """Execute all 50 Ehlers strategies on a limited set of symbols"""
    ...
    def get_limited_symbols(self) -> List[str]:
        """Get a limited set of symbols including RELIANCE and other major stocks"""
        # Focus on major stocks and RELIANCE
        target_symbols = [
        ...
        # Filter to only include symbols that exist in our data
        available_symbols = []
        for symbol in target_symbols:
            if symbol in all_symbols:
                available_symbols.append(symbol)
        
        # Add some additional random symbols to reach ~100 total
        remaining_symbols = [s for s in all_symbols if s not in available_symbols]
        additional_symbols = remaining_symbols[:50]  # Add 50 more random symbols
        
        final_symbols = available_symbols + additional_symbols
        
        print(f"Selected {len(final_symbols)} symbols (including {len(available_symbols)} major stocks)")
    ...
    def execute_limited_strategies(self) -> Dict[str, Any]:
        """Execute all 50 strategies on limited symbols"""
        print("Starting limited execution of all strategies on selected symbols...")
        
        # Get limited symbols
        symbols = self.get_limited_symbols()
        
        print(f"Executing {len(self.strategies)} strategies on {len(symbols)} symbols...")
        print(f"Total executions: {len(self.strategies) * len(symbols):,}")
    ...
    def create_summary_report(self):
        ...
        report = []
        report.append("EHLERS LIMITED EXECUTION SUMMARY REPORT")
        report.append("=" * 60)
        ...
        if reliance_trades['strategy_details']:
        ...
        # Save report
        with open('ehlers_limited_execution_report.txt', 'w') as f:
            f.write('\n'.join(report))
        
        print("Summary report created: ehlers_limited_execution_report.txt")

def main():
    """Main function to run limited execution"""
    print("EHLERS LIMITED EXECUTION - ALL STRATEGIES ON SELECTED SYMBOLS")
    print("=" * 60)
    
    # Initialize execution engine
    executor = EhlersLimitedExecution()
    
    # Execute all strategies on limited symbols
    results = executor.execute_limited_strategies()
    
    # Export results
    executor.export_results()
    
    print("\nLimited execution completed successfully!")
    print("Check the following files for results:")
    print("  - ehlers_limited_execution_aggregated.json")
    print("  - ehlers_reliance_trades_limited.json")
    print("  - ehlers_limited_execution_report.txt")
